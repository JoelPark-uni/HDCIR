...
class CIRCODataset(Dataset):
    ...
    def __getitem__(self, index) -> dict:
        ...
        if self.mode == 'relative':
            ...
            blip_ref_img_pil = PIL.Image.open(reference_img_path).convert('RGB')
...
class COCODataset(Dataset):
    ...
    def __init__(self, transform=None, root_dir='/shared-local/skarthik63/coco/val2017/') -> None:
        super().__init__()
    ...
    def load_blip_sample(self, sample):
        ...
        orig_img = PIL.Image.open(fpath).convert('RGB')
        return orig_img
...
class COCOValSubset(COCODataset):
    ...
    def __getitem__(self, index):
        ...
        blip_ref_img_pil = self.load_blip_sample(orig_reference)
        ...
        return reference, caption, blip_ref_img_pil, gallery_and_target,0  
...
class VAWDataset(Dataset):
    ...
    def __init__(self, transform=None, image_dir='/shared-local/skarthik63/Visual_Genome/VG_All/') -> None:
        super().__init__()
...
class ToyFashionIQDataset(Dataset):
    """
    ToyFashionIQ dataset class for PyTorch - loads from a single caption validation file.
    Similar to FashionIQDataset but uses a specific caption file (e.g., valid_cap.dress.val.json).
    The dataset can be used in 'relative' or 'classic' mode:
        - In 'classic' mode the dataset yield a dict with keys ['image', 'image_name']
        - In 'relative' mode the dataset yield dict with keys:
            - ['reference_image', 'reference_name', 'target_image', 'target_name', 'relative_captions']
    ...
    def __init__(self, dataset_path: Union[Path, str], caption_file: Union[Path, str],
                 mode: Literal['relative', 'classic'], preprocess: callable,
                 no_duplicates: Optional[bool] = False):
        """
        :param dataset_path: path to the FashionIQ dataset (for loading images)
        :param caption_file: path to the caption validation file (e.g., valid_cap.dress.val.json)
        :param mode: dataset mode, should be in ['relative', 'classic']:
            - In 'classic' mode the dataset yield a dict with keys ['image', 'image_name']
            - In 'relative' mode the dataset yield dict with keys:
                - ['reference_image', 'reference_name', 'target_image', 'target_name', 'relative_captions']
        :param preprocess: function which preprocesses the image
        :param no_duplicates: if True, the dataset will not yield duplicate images in relative mode
        :param blip_transform: function to preprocess images for BLIP
        """
        dataset_path = Path(dataset_path)
        ...
        # Load triplets from the caption file
        ...
        # Extract unique image names from triplets for classic mode
        self.image_names: list = []
        seen_images = set()
        for triplet in self.triplets:
            for img_name in [triplet['candidate'], triplet.get('target')]:
                if img_name and img_name not in seen_images:
                    seen_images.add(img_name)
                    self.image_names.append(img_name)

        print(f"ToyFashionIQ dataset from {caption_file.name} in {mode} mode initialized ({len(self.triplets)} triplets, {len(self.image_names)} unique images)")
